...
public class ValidationFormationGeologue extends ValidationFormation {
    ...
    public static boolean possedeHeureFormationMinimum(JSONObject info) {
        return ((Calcul.getHeures_Categories(info, "cours") >= 22)
                && Calcul.getHeures_Categories(info, "projet de recherche") >= 3
                && Calcul.getHeures_Categories(info, "groupe de discussion") >= 1);
    }
    ...
    public static boolean possedeNombreHeuresTotalSuffisant(JSONObject info) {
        return (Calcul.getHeuresTotal(info) >= 55);
    }
    ...
    public static ArrayList<String> getErreurs(JSONObject info) {
        ...
        if (possedeNombreHeuresTotalSuffisant(info)) {
            erreur.add("Il manque " + (55 - Calcul.getHeuresTotal(info)) + " heures pour compléter le cycle");
        }
        if (!possedeHeureFormationMinimum(info)) {
            erreur.add(MSG_ERR_MIN_HEURE1 + " 26 " + MSG_ERR_MIN_HEURE_GEOLOGUE);
        }
