...
public class ValidationFormationArchitecte extends ValidationFormation {
    ...
    public static boolean possedeNombreHeuresTotalSuffisant(JSONObject info) {
        ...
        return ((cycle.equals("2018-2020") && (Calcul.getHeuresTotal(info) > 40)) ||
                Calcul.getHeuresTotal(info) >= 42);
    ...
    public static boolean possedeHeureFormationMinimum(JSONObject info) {
        return (Calcul.heuresCategoriesMinimumArchitecte(info) >= 17);
    }
    ...
    public static ArrayList<String> getErreurs(JSONObject info) {
        ...
        if (!(possedeHeureTransfereValide(info))) {
            erreur.add(MSG_ERR_HEURES_PRECEDENTES);
        }
        for (int i = 0; i < Formation.length; i++) {
            if (!(estCategorieValide(Formation[i]))) {
        ...
        if (!(possedeNombreHeuresTotalSuffisant(info))) {
            erreur.add("Il manque " + (40 - Calcul.getHeuresTotal(info)) + " heures pour compléter le cycle");
        }
        if (!possedeHeureFormationMinimum(info)) {
            erreur.add(MSG_ERR_MIN_HEURE1 + " 17 " + MSG_ERR_MIN_HEURE_ARCHITECTE);
        }
